...
Must use environment variables (`.env` / `app.config.ts` extra fields via `expo-constants`, EAS secrets for build-time values). Allowed to commit: `.env.example`, config templates.
May NOT commit: `.env`, `.env.local`, `.env.production` (unless Conductor-approved).

**On-device secret storage — MANDATORY**

Auth tokens, session identifiers, and any other sensitive value persisted on the device must go through `expo-secure-store` (Keychain/Keystore-backed), never through `react-native-mmkv` or `AsyncStorage` in plaintext. MMKV is fine for non-sensitive app state (UI prefs, caches).

```bash
grep -rn "AsyncStorage\|mmkv" apps/*/src --include="*.ts" --include="*.tsx" -A2 -B2
```
Any hit that stores a token, password, or credential outside `expo-secure-store` → fix before gate-out.
...
Workers may NOT create new public-facing interfaces (REST, GraphQL, WebSocket, RPC, webhooks, auth/admin endpoints, event streams) unless explicitly defined in ARCHITECTURE.md and CONTRACTS.md and approved by the Conductor.
...
Workers may NOT expose customer data, personal data (KYC/PII for a payment app is especially sensitive — names, national ID, bank account/slip data), confidential information, credentials, or audit information.
...
Backend/API stages must also verify: no undocumented routes/APIs/debug/bypass-auth/direct-DB-exposure exist; all routes defined in CONTRACTS.md and approved in ARCHITECTURE.md.

All stages must verify:
* no secrets committed
* no tokens/credentials stored outside `expo-secure-store`
